...
// Public, minimal endpoint used only to populate the login screen's user
// dropdown before anyone is authenticated. Deliberately returns nothing
// beyond username/name/role — no balances, ledgers, account numbers, or
// password hashes ever appear here.
app.get('/api/login-users', (req, res) => {
  try {
    const state = readState();
    if (!state || !state.users) return res.json({ users: [] });
    if (migrateUsers(state)) writeState(state);
    const users = Object.entries(state.users).map(([key, u]) => ({
      key,
      name: u.name,
      role: u.role,
    }));
