...
// The branch/site label shown in the app's header (e.g. "Сарраёсат",
// "Филиали ..."). Set via the BRANCH_NAME environment variable
// per-deployment, so each separate installation can be told apart at a
// glance. Empty by default (shows nothing extra).
const BRANCH_NAME = process.env.BRANCH_NAME || '';

app.get('/', (req, res) => {
  fs.readFile(path.join(__dirname, 'index.html'), 'utf8', (err, html) => {
    if (err) return res.status(500).send('Could not load app');
    res.send(html.replace('%%BRANCH_NAME%%', BRANCH_NAME.replace(/[<>&]/g, '')));
  });
});
